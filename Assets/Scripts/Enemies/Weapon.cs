...
public class Weapon : CollideCharacters
{
    ...
    protected override void OnCollide(Collider2D coll)
    {
        ...
        if (coll.tag == "Fighter" && coll.name != "Player_0")
        {
            ...
            {
                damageAmount = (damage[weaponLevel] + playerStats.damage.GetValue()) * playerStats.damageModifier.GetValue(),
                origin = transform.position,
                knockback = knockback[weaponLevel] * playerStats.knockbackModifier.GetValue()
            };
