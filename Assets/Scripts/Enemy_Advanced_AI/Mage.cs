...
public class Mage : Fighter
{
    ...
    private void Update()
    {
        ...
        {
            //if (x >= 1 && x <= 100)
            if (target.position.y >= transform.position.y - 0.1f && target.position.y <= transform.position.y + 0.1f)
            {
    ...
    void Shoot()
    {
        ...
        GameObject Mage_Fireball = Instantiate(fireball, projectilePoint.position, newRotation);
        Rigidbody2D rb = Mage_Fireball.GetComponent<Rigidbody2D>();
